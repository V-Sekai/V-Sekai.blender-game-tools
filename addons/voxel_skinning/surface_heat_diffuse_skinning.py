...
class SFC_OT_ModalTimerOperator(bpy.types.Operator):
    ...
    def write_bone_data(self, obj, filepath):
        ...
        for bone in amt.edit_bones:
            if bone.use_deform:
                ...
                if bpy.context.scene.surface_use_selected_bones and not bone.select:
                    continue
    ...
    def execute(self, context):
        ...
        if not self.write_bone_data(arm, os.path.join(os.path.dirname(__file__), "data", "untitled-bone.txt")):
            if bpy.context.scene.surface_use_selected_bones:
        ...
        if platform.system() == 'Windows':
            if platform.machine().lower().endswith('amd64') or platform.machine().lower().endswith('x86_64'):
                ...
                executable_path = os.path.join(os.path.dirname(__file__), "bin", platform.system(), "arm64", "shd")
            else:
        ...
        self._pid = Popen([executable_path,
                        ...
                        "y" if context.scene.surface_use_half_cpu_cores else "n"],
        ...
        bpy.context.scene.surface_job_finished = False
    ...
    def cancel(self, context):
        ...
        bpy.context.scene.surface_job_finished = True
...
def init_properties():
    ...
    bpy.types.Scene.surface_job_finished = BoolProperty(
        name = "Surface Job Finished",
    ...
    bpy.types.Scene.surface_use_selected_bones = BoolProperty(
        name = "Use Selected Bones",
        description = "Use only selected bones",
        default = False)

    bpy.types.Scene.surface_use_half_cpu_cores = BoolProperty(
...
def clear_properties():
    ...
    "surface_protect",
    "surface_sharpness",
    "surface_job_finished",
    "detect_surface_solidify",
    "surface_use_selected_bones",
    "surface_use_half_cpu_cores"]
...
class SFC_PT_SurfaceHeatDiffuseSkinningPanel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        layout.prop(context.scene, 'surface_use_selected_bones')
        layout.prop(context.scene, 'surface_use_half_cpu_cores')

        row = layout.row()
        row.operator("wm.surface_heat_diffuse")


def register_surface_heat_diffuse_skinning():
    bpy.utils.register_class(SFC_PT_SurfaceHeatDiffuseSkinningPanel)
    bpy.utils.register_class(SFC_OT_ModalTimerOperator)
    init_properties()


def unregister_surface_heat_diffuse_skinning():
