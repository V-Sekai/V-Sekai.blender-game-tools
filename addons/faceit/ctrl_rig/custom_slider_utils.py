...
def generate_extra_sliders(context, shape_name, slider_range, rig_obj, max_rows=10, overwrite=False):
    ...
    def new_bone_pair_at_location(name, pos, overwrite=False):
        ...
        if any([b != None for b in slider_bones]) and overwrite:
        ...
        # Create new bones
        if not slider_bones or all([b == None for b in slider_bones]):
        ...
        elif any([b != None for b in slider_bones]) and not overwrite:
