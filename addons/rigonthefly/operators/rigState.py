...
class SaveRigStateOperator(bpy.types.Operator, ExportHelper):
    ...
    def invoke(self, context, event):
        currentFolderPath = context.scene.get('rotf_folder_path')
        if currentFolderPath:
            self.filepath = currentFolderPath + '\\'
...
class RefreshFilePathOperator(bpy.types.Operator):
    bl_idname = 'rotf.refresh_file_path'
    bl_label = 'Refresh File Path'
    bl_description = "Refresh folder path where rig states are located"
    bl_options = {'REGISTER', 'UNDO', 'INTERNAL'}

    def execute(self, context):
        folderPath = context.scene['rotf_folder_path']
        folderName = context.scene['rotf_folder_name']
