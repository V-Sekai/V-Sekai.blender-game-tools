...
class RemoveProxyOperator(bpy.types.Operator):
    bl_idname = "rotf.remove_proxy"
    bl_label = "Remove Proxy"
    bl_description = "Applies the motion to the linked armature and removes the duplicaded armatures"
    bl_options = {'REGISTER', 'UNDO', 'INTERNAL'}

    def execute(self, context):
        result = proxy.RemoveProxy()
        if result != None:
            self.report(*result) # * unpacks list into a tuple
            return {'CANCELLED'}
        return {'FINISHED'}
...
class RootMotionOperator(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if result != None:
            self.report({'WARNING'}, result) # * unpacks list into a tuple
        return {'FINISHED'}
