        ...
        for constraint in pbone.constraints:
            if " Simple RotF" in constraint.name:
                ...
                if constraint.type == 'COPY_LOCATION':
                    bonesToBakeInfo[pbone] = [[targetPBone, rotfBake.Channel.locationXYZ, rotfBake.Channel.locationXYZ]]
                    if "Simple Copy Transforms|"+ pbone.name not in constraintToRemoveList:
                        constraintToRemoveList.append("Simple Copy Transforms|"+ pbone.name)

                if constraint.type == 'COPY_ROTATION':
                    bonesToBakeInfo[pbone] = [[targetPBone, rotfBake.Channel.rotationQE, rotfBake.Channel.rotationQE]]
                    if "Simple Copy Transforms|"+ pbone.name not in constraintToRemoveList:
                        constraintToRemoveList.append("Simple Copy Transforms|"+ pbone.name)

                if constraint.type == 'COPY_SCALE':
                    bonesToBakeInfo[pbone] = [[targetPBone, rotfBake.Channel.scaleXYZ, rotfBake.Channel.scaleXYZ]]
                    if "Simple Copy Transforms|"+ pbone.name not in constraintToRemoveList:
                        constraintToRemoveList.append("Simple Copy Transforms|"+ pbone.name)
                
                if constraint.type == 'DAMPED_TRACK':
                    bonesToBakeInfo[pbone] = [[targetPBone, rotfBake.Channel.locationXYZ, rotfBake.Channel.rotationQE]]
                    if "Simple Aim|"+ pbone.name not in constraintToRemoveList:
                        constraintToRemoveList.append("Simple Aim|"+ pbone.name)
