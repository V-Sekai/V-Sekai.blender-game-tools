...
def SetupRotationDistributionControllers(rotDistSettingsList):
    ...
    for rotDistSettings in rotDistSettingsList:
        ...
        newBones, newEditBones, newBoneNames = duplicateBone.DuplicateBones("RotDistribution.", [targetEBone])
    ...
    for rotDistSettings in rotDistSettingsList:
        ...
        rotDist_customShape = bpy.context.scene.rotf_rotationDistribution_customShape
        ...
        rotDistPBone.bone.show_wire = False
        ...
        bonesToBakeInfo[rotDistPBone] = [targetPBone]
...
def SetupRotationDistributionBehaviour(rotDistSettingsList):
    for rotDistSettings in rotDistSettingsList:
        ...
        secondaryRotDistPBoneList = list()
        for i, pbone in zip(range(rotDistSettings.chainLength - 1), targetPBone.parent_recursive):
            pboneToMoveList.append(pbone)
            secondaryRotDistPBoneList.append(pbone) 
        ...
        if appVersion[0] == 4:
            for pbone in pboneToMoveList:
                ...
                boneCollections.AddBoneToCollections(pbone.bone, [boneCollections.RotFUnusedColName, 
                                                                  boneCollections.RotFHiddenRotDistColName])
            for pbone in secondaryRotDistPBoneList:
                boneCollections.AddBoneToCollections(pbone.bone, [boneCollections.RotFSecondaryRotDistColName])
...
def RemoveRotationDistribution(pboneWithRotDistList):
    ...
    for targetPBone in pboneWithRotDistList:
        ...
        bonesToBakeInfo[targetPBone] = [rotDistPBone]

        for pbone in pboneChainList:
            bonesToBakeInfo[pbone] = [rotDistPBone]
        ...
        if appVersion[0] == 4:
            for pbone in [targetPBone]+pboneChainList:
                ...
                boneCollections.UnassignBoneFromCollections(pbone.bone, [boneCollections.RotFUnusedColName, 
                                                                  boneCollections.RotFHiddenRotDistColName,
                                                                  boneCollections.RotFSecondaryRotDistColName])
