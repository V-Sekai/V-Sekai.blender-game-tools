...
def SetupParentSpaceControllers(parentSettingsList):
    ...
    for parentSettings in parentSettingsList:
        ...
        if parentCopy: #if parent copy is true
            if obj.name +"|ParentCopy."+targetParentBoneN in existingParentCopyNList: #if the parent copy bone already exists in the armature
                ...
                newParentBones, newParentEditBones, newParentBoneNames = duplicateBone.DuplicateBones("ParentCopy.", [targetParentEBone])
        ...
        newBones, newEditBones, newBoneNames = duplicateBone.DuplicateBones("Child.", [targetEBone])
    ...
    for parentSettings in parentSettingsList:
        ...
        if parentSettings.parentCopy:
            ...
            bonesToBakeInfo[parentPBone] = [targetParentPBone]
        ...
        bonesToBakeInfo[childPBone] = [targetPbone]
...
def RemoveParent(targetPoseBonesWithParent):
    ...
    parentPBoneList = list()
    for targetPBone in targetPoseBonesWithParent:
        ...
        if appVersion[0] == 4:
            ...
            #boneCollections.AddBoneToCollections(targetBoneP.bone, [boneCollections.RotFAnimationColName])
            boneCollections.UnassignBoneFromCollections(targetPBone.bone, [boneCollections.RotFUnusedColName])

        elif appVersion[0] == 3:
            for layer in range(32):
                targetPBone.bone.layers[layer] = childTargetPBone.bone.layers[layer]

        bonesToBakeInfo[targetPBone] = [childTargetPBone]
    ...
    for targetPBone, copyTransforms in zip(targetPoseBonesWithParent, copyTransformsConstraintList):
    ...
    for targetPBone in targetPoseBonesWithParent:
        rigState.RemoveConstraint(obj, "Parent Space|"+ targetPBone.name)
        rigState.RemoveConstraint(obj, "Parent Offset Space|"+ targetPBone.name)
